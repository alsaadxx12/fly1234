...
import { Search, Package, X, ChevronDown, DollarSign } from 'lucide-react';
...
export default function SourceSelector({ value, onChange, sources = [], required, label = 'المصدر' }: SourceSelectorProps) {
  ...
  const handleSelect = (source: Source) => {
    onChange(source.name, source.currency);
    setIsOpen(false);
    setSearchTerm('');
    setHighlightedIndex(-1);
  };
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div className="relative">
        ...
        {isOpen && searchTerm.length >= 3 && filteredSources.length > 0 && (
          <div
            ref={dropdownRef}
              ...
              >
                ...
                <div className="flex-1 text-right">
                  ...
                  <div className={`flex items-center justify-end gap-1 text-xs font-bold mt-0.5 ${
                    ...
                    {source.currency === 'IQD' ? 'دينار عراقي' : 'دولار أمريكي'}
                  </div>
                </div>
              </button>
            ))}
