      ...
      <div className="relative p-6 pb-0">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-black opacity-60 uppercase tracking-wider">{dayName}</span>
              <span className="text-xs font-bold opacity-40">{fullDate}</span>
            </div>
          </div>
        ...
        <div className="flex flex-col items-center text-center">
          <div className="relative group">
            <div className={`absolute inset-0 rounded-full blur-2xl transition-all duration-500 opacity-20 group-hover:opacity-40 animate-pulse ${isCheckedIn ? 'bg-rose-500' : 'bg-emerald-500'
              }`} />
            <div className={`relative w-28 h-28 rounded-full p-1 border-2 transition-all duration-500 ${isCheckedIn ? 'border-rose-500/50' : 'border-emerald-500/50'
              }`}>
              <div className="w-full h-full rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden border-4 border-transparent shadow-inner">
                {employee?.image ? (
          ...
          <h2 className="text-3xl font-black mt-6 tracking-tight">{employee?.name}</h2>
          <p className="text-sm font-bold opacity-40 mt-1 max-w-[200px] truncate">{employee?.email}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className={`p-4 rounded-[2rem] border transition-all duration-300 hover:scale-[1.02] ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-blue-50/50 border-blue-100/50'
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-tight opacity-50">القسم</span>
            </div>
            <p className="text-sm font-bold truncate leading-none">{assignedDepartment?.name || '---'}</p>
          </div>

          <div className={`p-4 rounded-[2rem] border transition-all duration-300 hover:scale-[1.02] ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-purple-50/50 border-purple-100/50'
            }`}>
            <div className="flex items-center gap-3 mb-2">
              <MapPin className="w-4 h-4 text-purple-500" />
              <span className="text-[10px] font-black uppercase tracking-tight opacity-50">الفرع</span>
            </div>
            <p className="text-sm font-bold truncate leading-none">{assignedBranch?.name || '---'}</p>
          </div>
        </div>

        {/* Status Timeline */}
        <div className={`p-6 rounded-[2.5rem] border ${theme === 'dark' ? 'bg-black/20 border-white/5' : 'bg-gray-50/50 border-gray-200/50'
          }`}>
          {isCheckedIn ? (
            ...
            <div className="flex items-center gap-6">
              <div className="w-14 h-14 rounded-2xl bg-gray-500/10 flex items-center justify-center text-gray-500 border border-gray-500/20 shadow-lg shadow-gray-500/5">
                <LogOutIcon className="w-6 h-6" />
              </div>
              <div className="flex flex-col">
                <span className="text-xs font-black opacity-40 uppercase tracking-widest mb-1">آخر تسجيل خروج</span>
                <span className="text-xl font-black tracking-tighter">
          ...
          <div className="relative group">
            ...
            >
              {loading ? (
                ...
                <Fingerprint className="w-14 h-14 text-white mb-2" />
              )}
            </button>
          </div>

          <div className="mt-6 flex flex-col items-center">
            <span className={`text-lg font-black tracking-tight ${isCheckedIn ? 'text-rose-500' : 'text-emerald-500'}`}>
              {loading ? 'جاري التحميل...' : isCheckedIn ? 'تسجيل الخروج' : 'تسجيل الحضور'}
            </span>
            {!loading && (
