...
interface VisaBeneficiarySelectorProps {
  value: string;
...
const VisaBeneficiarySelector = React.memo(function VisaBeneficiarySelector({
  value,
  onChange,
  companies = [],
  required,
    ...
    <div ref={containerRef} className="relative h-full">
      <div className="relative h-full">
        <div className={`flex items-center gap-2 w-full h-full px-3 rounded-xl border-2 transition-all ${
          isOpen
            ? theme === 'dark'
              ? 'bg-gray-900/50 border-emerald-500 ring-2 ring-emerald-500/30'
          ...
          <input
            ...
            onFocus={() => {
              setSearchTerm(value);
              if (value.length >= 1 || companies.length > 0) {
                setIsOpen(true);
              }
            }}
...
{isOpen && filteredCompanies.length > 0 && (
          <div
            ref={dropdownRef}
            className={`absolute z-[100] w-full mt-1 rounded-xl border-2 shadow-xl max-h-64 overflow-y-auto custom-scrollbar ${
              theme === 'dark'
            ...
            style={{
              scrollbarWidth: 'thin',
              scrollbarColor: theme === 'dark' ? '#374151 #1f2937' : '#d1d5db #f3f4f6'
            }}
          >
            {filteredCompanies.slice(0, 100).map((company, index) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-bold transition-colors border-b last:border-b-0 ${
                  highlightedIndex === index
                    ...
                    : theme === 'dark'
                      ...
                      : 'hover:bg-emerald-50 border-gray-100 text-gray-900'
                }`}
            ...
            {filteredCompanies.length > 100 && (
              <div className={`px-4 py-2 text-xs text-center ${
                ...
                عرض أول 100 نتيجة - استخدم البحث للتصفية
              </div>
            )}
          </div>
        )}

        {isOpen && filteredCompanies.length === 0 && searchTerm.length > 0 && (
...
export default VisaBeneficiarySelector;
