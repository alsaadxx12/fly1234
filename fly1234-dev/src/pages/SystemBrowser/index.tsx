...
const SystemBrowser: React.FC = () => {
    ...
    const fetchAllData = useCallback(async () => {
        ...
        const MAX_RETRIES = 3;
        const perpage = 1500; // Large chunk size for "single request" feel, with retries for safety
        let page = 1;
        const allItems: any[] = [];
        let total = 0;
        ...
        try {
            while (true) {
                ...
                allItems.push(...items);
                setRawUsers([...allItems]); // Incremental update for responsiveness
    ...
    const exportData = (type: 'excel' | 'json') => {
        ...
        try {
            ...
            // Fallback: If nothing selected, export all filtered
            const finalData = dataToExport.length > 0 ? dataToExport : filteredUsers;

            console.log(`Data to export length: ${finalData.length}`);
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 transition-colors">
            {/* Ultra-Compact Search & Tools Row */}
            <div className="flex-shrink-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-3 flex items-center gap-3 z-20">
            ...
            <div className="flex-shrink-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 px-6 py-3 flex items-center justify-between gap-3 z-10 shadow-sm">
                <div className="flex items-center gap-2">
                    <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 w-36 focus-within:ring-1 focus-within:ring-blue-500/20 transition-all">
            ...
            <div className="flex-1 overflow-hidden flex flex-col relative">
                ...
                <div className="bg-white/50 dark:bg-slate-900/30 backdrop-blur-sm border-b border-slate-200 dark:border-slate-800 px-6 py-2 flex items-center justify-between gap-4 z-10">
                    ...
                    <div className="text-[9px] font-black text-slate-400 uppercase bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">
                        الإجمالي: <span className="text-slate-900 dark:text-white">{filteredUsers.length.toLocaleString()}</span>
                    </div>
                </div>
