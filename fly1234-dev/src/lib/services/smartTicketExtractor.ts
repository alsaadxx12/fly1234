...
/**
 * ✅ يستخرج تاريخ جلالي كامل من داخل النص مثل: 1404/10/20 أو 1404-10-20
 * ويرجعه كميلادي ISO: YYYY-MM-DD
 */
function extractFullJalaliDateFromText(rawText: string): string | undefined {
  ...
  // يلتقط داخل أي نص: 13xx/xx/xx أو 14xx/xx/xx
  ...
  const g = moment(`${jy}/${jm}/${jd}`, "jYYYY/jM/jD", true);
...
/**
 * ✅ يقرأ "19 دی" من النص (فارسي/جلالي) ويحوّله إلى ميلادي
 * ✅ بدون تحويل للسنة القادمة نهائياً (يستخدم السنة الجلالية الحالية فقط)
 */
function extractJalaliDayMonthFromText(rawText: string): string | undefined {
  ...
  // ✅ السنة الجلالية الحالية فقط (بدون أي قفزة للسنة القادمة)
  const nowGregorian = new Date('2026-01-16T12:00:00Z');
  const nowJalali = moment(nowGregorian).jDateObject();
  const jy = nowJalali.jy;

  try {
    const gregorianDate = moment(`${jy}/${month}/${day} 12:00`, 'jYYYY/jM/jD HH:mm', true);
...
function normalizeDateToISO(input?: string): string | undefined {
  ...
  // ✅ 1) إذا النص يحتوي تاريخ جلالي كامل داخل النص
  ...
  if (mYMD) {
    ...
    // جلالي
    if (y >= 1300 && y <= 1500) {
      const g = moment(`${y}/${mo}/${d}`, 'jYYYY/jM/jD', true);
      if (!g.isValid()) return undefined;
      return g.format('YYYY-MM-DD');
    }
...
export class SmartTicketExtractor {

  private static async getApiKey(): Promise<string> {
    try {
      const settingsRef = doc(db, 'settings', 'api_config');
      const settingsDoc = await getDoc(settingsRef);
      if (settingsDoc.exists() && settingsDoc.data().aiApiKey) {
        return settingsDoc.data().aiApiKey;
      }
  ...
  public static async extractAndTranslate(text: string): Promise<{ extractedData: ExtractedData, translatedText: string }> {
    ...
    // ✅ أفضل أولوية: تاريخ جلالي كامل مثل 1404/10/20
    const fullJalaliISO = extractFullJalaliDateFromText(text);

    // ✅ ثاني أولوية: يوم + شهر مثل 19 دی (بدون أي سنة قادمة)
    const jalaliHintISO = fullJalaliISO || extractJalaliDayMonthFromText(text);

    // Give AI context about today's date.
    const todayISO = new Date('2026-01-16T12:00:00Z').toISOString().slice(0, 10);
    ...
    try {
      ...
      // ✅ القرار النهائي للتاريخ: النص أولاً ثم AI
      const aiDateISO = normalizeDateToISO(String(obj.date || obj.flightDate || ""));
      const finalDateISO = jalaliHintISO || aiDateISO || '';

      const extractedData: ExtractedData = {
        airline: String(obj.airline || "").trim(),
        flightNumber: String(obj.flightNumber || obj.flight_no || "").trim(),
        date: finalDateISO,
        origin: toIata(String(obj.origin || "")),
        destination: toIata(String(obj.destination || "")),
        type: inferredType as ExtractedData['type'],
        oldTime: oldTime || "",
        newTime: newTime || "",
        newFlightNumber: newFlightNumber,
        newAirline: String(obj.newAirline || "").trim(),
        pnr: String(obj.pnr || "").trim(),
        passengers: []
      };
  ...
  public static async extractFromPDF(file: File): Promise<ExtractedData> {
    return {} as ExtractedData; // Placeholder
  }

  public static async extractFromImage(file: File): Promise<ExtractedData> {
    return {} as ExtractedData; // Placeholder
  }
}
